...
class MCBot {
    ...
    initBot() {
        ...
        this.bot.once('spawn', () => {
            if (config.noChat === true) {
                ...
                this.bot.chat(`Minebot ${version}. Entwickelt Von Eglijohn.`);
            }
        });
    }
    ...
    async executeCommand(command) {
        ...
        switch (cmd) {
            case 'help':
                this.log(`[LOG] !setowner <name>, !quit, !players. [DM] = Direktnachricht (/tell, /msg), [PC] = Öffentlicher Chat.`);
                break;

            case 'players':
                const playerList = Object.keys(this.bot.players).filter(player => player !== this.bot.username);
                if (playerList.length > 0) {
                    this.log(`[LOG] Online Spieler: ${playerList.join(', ')}`);
                } else {
                    this.log(`[LOG] Keine Anderen Spieler Sind Online.`);
                }
                break;
            ...
            case 'setowner':
                ...
                owner = newOwner;
                this.log(`[LOG] Neuer Owner Ist jetzt: ${owner}`);
                this.bot.chat(`/msg ${owner} Herzlichen Glückwunsch, Sie sind jetzt der neue Benutzer!`);
                break;

            case 'say':
                this.log('[LOG] Versuchen Sie, den Text ohne Syntax einzugeben, bevor Sie.')

            case 'follow':
            case 'stopfollow':
            case 'mine':
                this.log(`[LOG] Command '${cmd}' muss über den Minecraft-Chat verwendet werden.`);
                break;

            default:
                this.log(`[LOG] Der Command Wurde Nicht Gefunden '${cmd}'. Benutze !help für die Commnd Liste.`);
                break;
        }
    }
    ...
    initEvents() {
        this.bot.on('login', async () => {
            ...
            this.log(`[LOG] Ist Eingelogt in ${botSocket.server ? botSocket.server : botSocket._host}`);
        });

        this.bot.on('end', async (reason) => {
            const logMessage = `[LOG] Verbindung Abgebrochen: ${reason}`;
            fs.appendFileSync('log.txt', logMessage + '\n');
            console.log(logMessage);
